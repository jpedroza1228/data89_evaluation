...
att2_name = 'Calculating & Probability'
...
alpha = alpha.rename(columns = {0: att1_name,
                                1: att2_name}).clean_names(case_type = 'snake')
...
```{python}
# did students not slip/true positive (get answer correct without knowing content)
...
```{python}
attr_mastery[['attr1', 'attr2']].value_counts().reset_index()
```
