...
alpha = alpha.rename(columns = {0: 'Rules, Logic, & Sets',
                                1: 'Bounding Chances',
                                2: 'Probability & Computations'}).clean_names(case_type = 'snake')
...
q = pd.read_csv(here('data/q_matrix/q1_7item_3att.csv')).clean_names(case_type = 'snake')
...
dino = joblib.load(here('joblib_models/quiz1_model_modfit.joblib'))
dino_prior = joblib.load(here('joblib_models/quiz1_model_modfit_prior_only.joblib'))


dinodf = dino[1].draws_pd()
# dinodf2 = dino_prior[1].draws_pd()
