// GDINA Model 7: General Diagnostic Classification Model with Random Effects
// This model includes random effects for items to account for item-level variation
...
parameters {
  ...
  vector[J] item_random_effect;      // random effect for each item
  real<lower=0> sigma_item;          // variance of item random effects
}
...
transformed parameters {
  ...
  for (n in 1:N) {
    for (p in 1:max_pattern) {
      ...
      for (j in 1:J) {
        real prob_correct = inv_logit(logit(delta[j, p]) + item_random_effect[j]);
        if (Y[n, j] == 1) {
...
model {
  ...
  item_random_effect ~ normal(0, sigma_item);
  sigma_item ~ cauchy(0, 1);
