...
stan_file = os.path.join(here('stan_models/bn_lcdm_mimic.stan'))
...
pd.Series(stan_fit.summary()['R_hat'].sort_values(ascending = False)).to_csv(here('stan_models/stan_output/rhat_values/bn_rhat_values.csv'))

(
  joblib.dump([stan_model, stan_fit],
              'stan_models/stan_output/bn_quiz1_fit.joblib',
              compress = 3)
)
