...
pi_df = pi_df.melt()
pi_df[['item', 'lat_class']] = pi_df['variable'].str.split(',', expand = True)
pi_df['item'] = pi_df['item'].str.replace('pi[', '').astype(int)
pi_df['lat_class'] = pi_df['lat_class'].str.replace(']', '').astype(int)
pi_df = pi_df[['item', 'lat_class', 'value']]

pi_df.loc[(pi_df['item'] == 1) &
          (pi_df['lat_class'] == 1)]
